...
public final class DamageNumbersConfigScreen extends DamageNumbersScreenBase {
    ...
    private static void applyFilter(EditBox box, boolean decimal) {
        // 26.x dropped the input filter hook; the parsers reject anything that is not a number.
    }

    private void openScreen(Screen screen) {
        flush();
        ScreenNavigator.open(minecraft, screen);
    }
     ...
     */
    private void renderTextFields(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick,
                                  int from, int to) {
        if (openSelect != null) {
            return;
        }
     ...
     */
    private void pushPopupLayer(GuiGraphicsExtractor graphics) {
    }

    private void popPopupLayer(GuiGraphicsExtractor graphics) {
    }
    ...
    private void renderSidebar(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        graphics.text(font, Component.empty().append(title).append(" 1.1"), 20, 21, TEXT, false);
        graphics.text(font, "by mel1x", 20, 33, FAINT, false);
        ...
        for (int index = 0; index < pages.length; index++) {
            ...
            graphics.text(font, fitLabel(Component.translatable(pages[index].translationKey),
        ...
        graphics.centeredText(font, Component.translatable("gui.done"), (right - 18) / 2 + 9,
                doneY + 8, 0xFFFFFFFF);
    }

    private void renderHeader(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        int left = contentX();
        graphics.text(font, Component.translatable(page.translationKey), left, 18, TEXT, false);
        graphics.text(font, fitLabel(Component.translatable(page.descriptionKey),
        ...
        if (page == Page.PRESETS) {
            ...
            graphics.centeredText(font, fitLabel(
    ...
    private void renderRangeStrip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        List<DamageRange> ranges = config.damageRanges();
        int active = config.activeDamageRangeIndex();
        ...
        for (RangeSegment segment : rangeSegments) {
            ...
            graphics.centeredText(font, fitLabel(Component.literal(
    ...
    private void renderContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        pose.pushMatrix();
        pose.translate(0.0F, -scrollFraction);
        for (Row row : rows) {
    ...
    private void renderPopup(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        for (int index = 0; index < options.size(); index++) {
            ...
            graphics.text(font, fitLabel(options.get(index), popupWidth - 26), x + 15,
    ...
    private abstract class Row {
        ...
        abstract void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY);
    }
    ...
    private final class SectionRow extends Row {
        ...
        @Override
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            int baseline = screenTop + height - 12;
            graphics.text(font, title, contentX() + 2, baseline, MUTED, false);
    ...
    private final class HintRow extends Row {
        ...
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            ...
            graphics.text(font, fitLabel(message, rowWidth() - 24), contentX() + 12,
    ...
    private final class ExpanderRow extends Row {
        ...
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            ...
            graphics.text(font, title, contentX() + 18, screenTop + height / 2 - 4, color, false);
        }
    }
    ...
    private abstract class SettingRow extends Row {
        ...
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            ...
            graphics.text(font, fitLabel(label, labelWidth), contentX() + PADDING, labelY,
                    active ? TEXT : FAINT, false);
            if (description != null) {
                graphics.text(font, fitLabel(description, labelWidth), contentX() + PADDING,
        ...
        abstract void renderControl(GuiGraphicsExtractor graphics, int x, int y, int width, boolean active,
    ...
    private final class ToggleRow extends SettingRow {
        ...
        @Override
        void renderControl(GuiGraphicsExtractor graphics, int x, int y, int width, boolean active,
                           int mouseX, int mouseY) {
    ...
    private final class SelectRow extends SettingRow {
        ...
        void renderControl(GuiGraphicsExtractor graphics, int x, int y, int width, boolean active,
            ...
            graphics.text(font, fitLabel(options.get(index), width - 26), x + 8, y + 6,
                    active ? TEXT : FAINT, false);
        ...
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            ...
            for (int index = 0; index < cards.size(); index++) {
                ...
                if (card.addCard()) {
                    ...
                    graphics.centeredText(font, FontStyleResolver.component("123", card.font(),
                ...
                graphics.centeredText(font, fitLabel(card.name(), FONT_CARD_WIDTH - 10),
    ...
    private final class PresetGridRow extends Row {
        ...
        void render(GuiGraphicsExtractor graphics, int screenTop, int mouseX, int mouseY) {
            ...
            for (int index = 0; index < cards.size(); index++) {
                ...
                graphics.centeredText(font, fitLabel(card.name(), cardWidth - 12),
        ...
        private void drawCardAction(GuiGraphicsExtractor graphics, int x, int y, int mouseX, int mouseY,
                                    boolean remove) {
    ...
    private static void roundRect(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int radius,
                                  int color) {
    ...
    private static void cutRect(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int cut,
                                int color) {
        shapedRect(graphics, x, y, width, height, cut, true, color);
    }

    private static void shapedRect(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int corner,
    ...
    private static void roundPanel(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int radius,
                                   int fill, int border) {
        roundRect(graphics, x, y, width, height, radius, border);
        roundRect(graphics, x + 1, y + 1, width - 2, height - 2, Math.max(0, radius - 1), fill);
    }

    private static void roundOutline(GuiGraphicsExtractor graphics, int x, int y, int width, int height,
    ...
    private static void circle(GuiGraphicsExtractor graphics, float centerX, float centerY, float radius,
                               int color) {
    ...
    private void drawNavIcon(GuiGraphicsExtractor graphics, Page target, int centerX, int centerY, int color) {
        switch (target) {
            case PRESETS -> {
    ...
    private void renderPreviewPanel(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
    ...
    private void drawNumberPreview(GuiGraphicsExtractor graphics, Snapshot style, float centerX, float centerY,
                                   float scale) {
        drawNumberPreview(graphics, style, "8", centerX, centerY, scale, 1.0F);
    }

    private void drawNumberPreview(GuiGraphicsExtractor graphics, Snapshot style, String value, float centerX,
     ...
     */
    private void drawPreviewGlyph(GuiGraphicsExtractor graphics, Matrix3x2fStack pose, Component glyph, int glyphX,
